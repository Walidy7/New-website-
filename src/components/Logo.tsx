...
export const LogoEmblem: React.FC<LogoEmblemProps> = ({
  ...
  return (
    <div
      ...
      style={{ width, height }}
      aria-label="Upper Agent Logo Emblem"
    >
      {!imageFailed ? (
        <img
          src={`${import.meta.env.BASE_URL}logo.png`}
          alt="Upper Agent Emblem"
...
export const Logo: React.FC<LogoProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Brand Text: UPPER AGENT */}
      {showText && (
        <div className="flex flex-col text-left min-w-0">
          <div className="flex items-center gap-1 sm:gap-1.5 tracking-tight">
            <span
              className={`bg-gradient-to-b from-white via-slate-100 to-slate-200 bg-clip-text text-transparent font-black tracking-wider uppercase drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] ${textSizes[size].builder}`}
            >
              Upper
            </span>
            <span
              className={`bg-gradient-to-b from-blue-400 via-blue-500 to-indigo-500 bg-clip-text text-transparent font-black tracking-wider uppercase drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] ${textSizes[size].agent}`}
            >
              Agent
            </span>
          </div>
